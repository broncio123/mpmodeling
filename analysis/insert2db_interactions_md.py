...
pdb_paths_original = [x.rstrip()+'/complex/mdf/md_0mV_100000ps_After_prmd' for x in open(pdblist,'r').readlines()]
# Extract PDB names
...
for k in range(len(pdb_paths_original)):
	cmd = ['ln', '-s', pdb_paths_original[k], './md_100ns_'+pdbnames[k]]
	subprocess.call(cmd)
# List of modified paths after creating soft links to folders
pdb_paths = ['./md_100ns_'+pdbnames[k]+'/' for k in range(len(pdbnames))]
