...
if [ ! -f ${path}/confout.gro ]; then
    echo "confout.gro file not found"
fi
...
if [ ! -f ${path}/protein+lipids.pdb ]; then
...
if [ ! -f ${path}/protein+lipids+sol_layer.pdb ]; then
...
if [ ! -f ${path}/protein+lipids+sol_layer_altered.pdb ]; then
    pymol -qc ~/mpmodeling/protocols/sem/alter_solvation_layer.py -- ${path}/protein+lipids+sol_layer.pdb ${path}/protein+lipids+sol_layer_altered.pdb
fi

# 6. Add chain labels for residue lines
if [ ! -f ${path}/protein_chains+lipids+sol_layer_altered.pdb ]; then
    pymol -qc ~/mpmodeling/protocols/sem/label_chains4gro.py -- ${path}/protein+lipids+sol_layer_altered.pdb ${path}/protein_chains+lipids+sol_layer_altered.pdb
fi

# 7. Redefine box dimensions, centering system
read Lx Ly Lz<<<$(tail -1 ${path}/confout.gro)
Lz=10.0626 
...
if [ ! -f ${path}/protein+lipids+sol_RAW.pdb ]; then
...
Lx,Ly,Lz = box_dim
# Reduce filter threshold to avoid atom overlap due to PBC
vdw_rad = 0.105
Lx = Lx - vdw_rad
Ly = Ly - vdw_rad
Lz = Lz - vdw_rad
...
for i in range(0,N,3):
	x_OW,y_OW,z_OW = list(map(float,SOL_data[i].split()[-6:-3]))
	x_H1,y_H1,z_H1 = list(map(float,SOL_data[i+1].split()[-6:-3]))
	x_H2,y_H2,z_H2 = list(map(float,SOL_data[i+2].split()[-6:-3]))
	if (0 < x_OW < Lx) and (0 < x_H1 < Lx) and (0 < x_H2 < Lx):
		if ( 0 < y_OW < Ly) and (0 < y_H1 < Ly) and (0 < y_H2 < Ly):
			if (0 < z_OW < Lz) and (0 < z_H1 < Lz) and (0 < z_H2 < Lz):
